...
app.post('/push', (req, res) => { 
  // let sql0 = 'insert into cred values ("pratham","123");';
  ...
  db.query(sql2,(err,result) => { 
    ...
    if(result[0].userpass == entereduserpass){
      ...
      db.query(sql,(err1,results) => { 
        ...
        console.log(results);
        for(i in results){
          respsonses = respsonses + '<ul>'+ results[i].name +"\t | \t"+  results[i].email + "\t | \t"+  results[i].purpose +"\t | \t" + results[i].mobile +"\t | \t" + results[i].department + "\t | \t"+  results[i].dateofapp +"\t | \t" + results[i].time + "   "+"<button onclick=deleteit('"+results[i].email+ "')" + ""+">✔</button>"+'</ul>';
        }
        console.log(respsonses);
        res.send('<html><script>function deleteit(e){console.log("deleting : ", e)}</script><body>User Authicated <a href="http://127.0.0.1:5500/HMS/login.html">Click me to LogOut </a> <br> '+ respsonses +'</body></html>');
      });
